...
## assume df
#@ assign df
df['route']=df['route']=df['start_station_id'].astype(str).str.cat(df['end_station_id'].astype(str),sep='->')
df['route']
...
#@ assume df
df['trip_duration_sec'].max()/3600

#@ problem 9
## what is the median trip length, in minutes?
#@ assume df
df['trip_duration_sec'].max()/60
...
#@ assume df
(df.start_station_name.str.contains('BART')| df.end_station_name.str.contains('BART')).mean()
...
#@ assume df
df['age_group'].value_counts(normalize=True).sort_values()
df.info()
...
## Hint: use groupby().
#@ assume df
df.groupby('age_group')['trip_duration_sec'].mean()/60
...
## member_gender.
#@ assume df
df.groupby('member_gender')['trip_duration_sec'].median()/60
...
#@ assume df
df.info()
df.groupby(['age_group','member_gender'])['trip_min'].median()

#@ problem 33
## Compute the mean and median trip length (in minutes) for each age group.
#@ assume df
df.groupby('age_group')['trip_duration_sec'].aggregate([np.mean ,np.median])/60
...
## Use group_by().
#@ assume df
df.groupby ('BART') ['user_type'].value_counts(normalize=True)
...
#@ assume df
pd.crosstab(index=df['user_type'],
